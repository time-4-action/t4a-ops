...
| mariadb | `mariadb` (dnf) | t4a-t2 | 3306 (localhost) | Active | Database server for WordPress sites. Bare-metal install (not Docker). Per-site DBs: `wp_{sitename}`. |
...
| wordpress | WordPress (latest) | t4a-t2 | — | Active | Multi-site WordPress hosting. Bare-metal installs (not Docker). Each site in `/var/www/t4a/<domain>/`. Served by nginx + php-fpm. Block volume `/dev/vdc` mounted at `/var/www/t4a/`. See [runbook](../docs/runbooks/wordpress-multisite-setup.md). |
